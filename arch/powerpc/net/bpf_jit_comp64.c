...
static int bpf_jit_stack_local(struct codegen_context *ctx)
{
	if (bpf_has_stack_frame(ctx))
		...
		return -(8 + BPF_PPC_STACK_SAVE + 24);
}

int bpf_jit_stack_tailcallinfo_offset(struct codegen_context *ctx)
{
	return bpf_jit_stack_local(ctx) + BPF_PPC_STACK_SAVE + 24;
}
...
void bpf_jit_build_prologue(u32 *image, struct codegen_context *ctx)
{
	...
	if (!ctx->is_subprog) {
		...
		EMIT(PPC_RAW_LD(bpf_to_ppc(TMP_REG_1), bpf_to_ppc(TMP_REG_2), -(8)));
		EMIT(PPC_RAW_CMPLWI(bpf_to_ppc(TMP_REG_1), MAX_TAIL_CALL_CNT));
		PPC_COND_BRANCH(COND_GT, CTX_NIA(ctx) + 8);
		EMIT(PPC_RAW_ADDI(bpf_to_ppc(TMP_REG_1), bpf_to_ppc(TMP_REG_2), -(8)));
		EMIT(PPC_RAW_STD(bpf_to_ppc(TMP_REG_1), _R1, -(8)));
	}
